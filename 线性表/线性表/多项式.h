...
istream &operator>>(istream &in,Polynomal &x){
	Term *rear=x.getHead(); float c;  int e;
	while(1){
		cout<<"Input aterm(coef,exp):"<<endl;
		in>>c>>e;
		if(e<0)  break;
...
Polynomal operator + (Polynomal &A,Polynomal &B){   // 多项式加法 
	 Term *pa,*pb,*pc,*p; float temp;
	 Polynomal C;   pc=C.getHead();
...
Polynomal operator * (Polynomal &A,Polynomal &B){
	...
	if(AL!=-1||BL!=-1){
	    ...
	    for(i=0;i<=maxExp;i++){	
	        if(fabs(result[i])>0.0) pc=pc->InsertAfter(result[i],i);
	    }
	    delete []result;
	}
	pc->link=NULL;
