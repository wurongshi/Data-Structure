...
template<class T>
LinkedSet<T> LinkedSet<T>:: operator +(LinkedSet<T> &R){   //并集 
	SetNode<T> *pb=R.first->link;
...
template<class T>
LinkedSet<T> LinkedSet<T>:: operator *(LinkedSet<T> &R){   //交集 
	SetNode<T> *pb=R.first->link;
	SetNode<T> *pa=first->link;
	LinkedSet<T> temp;
...
ostream &operator <<(ostream &out,LinkedSet<T> &R){
	...
	R.Max(x);
	out<<endl<<"集合中的最大元素为："<<x<<endl;
	R.Min(x);
	out<<"集合中的最小元素为："<<x<<endl;
	return out;
}